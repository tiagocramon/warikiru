...
import { fetchExpenses, updateExpense } from '../services/expenseService'
import { useLocalToast } from '../hooks/useLocalToast'
import type { GroupMember, GroupWithMembers, Expense, ExpenseCategory } from '../types/database'
...
export default function EditExpensePage() {
  ...
  const loadExpenseData = useCallback(() => {
    ...
    Promise.all([
      ...
      if (groupRes.data) {
        const g = groupRes.data as GroupWithMembers
        loadedMembers = g.members
        setMembers(g.members)
      }

      const expense = (expensesRes.data as Expense[] | null)?.find(
        (e) => e.id === expenseId
      )
      if (expense) {
        const hasCustom = !!expense.custom_percentages
        const pcts: Record<string, number> = {}

        if (hasCustom) {
          Object.entries(expense.custom_percentages!).forEach(([k, v]) => {
            pcts[k] = Number(v)
          })
        } else {
        ...
        reset({
          ...
          category: (expense.category || 'outros') as ExpenseForm['category'],
          description: expense.description,
          date: expense.date,
          paidByMemberId: expense.paid_by_member_id,
          useCustomPercentages: hasCustom,
          customPercentages: pcts,
        })

        // Sync amountRaw from the loaded expense
        const cents = Math.round(expense.amount * 100).toString()
        setAmountRaw(cents)

        // Sync calendar to expense date
  ...
  useEffect(() => {
    loadExpenseData()
  }, [loadExpenseData, refreshKey])

  async function onSubmit(data: ExpenseForm) {
    setSubmitting(true)

    const parsedCustom = data.useCustomPercentages
    ...
    if (error) {
      setError('root', { message: 'Erro ao atualizar despesa. Tente novamente.' })
      showToast('Ops, tivemos um erro!', 'error')
      setSubmitting(false)
    } else {
      showToast('Despesa editada com sucesso!', 'success')
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-1 flex-col">
        ...
        <div className="flex flex-col gap-[20px] px-[20px] sm:px-0">
          ...
          <div className="flex flex-col gap-[4px]">
            <p className="text-[28px] font-normal leading-none text-[#F5F7FA]">Editar Despesa</p>
